...
In this version, I edited the code from edge detection version 1 to achive the distance from the next edge
...
float alfas_when_edge[10]; // Define an array to hold 10 float alfa values when edge is detected values
...
float get_next_holes_from_laserscan(float angle_lidar, float distance_lidar, int scan_height){
  ...
  if(distance_cm > tolerance * distance_calculated_cm){
    ...
    float min_alfa_Value = findMin(alfas_when_edge, 10);
